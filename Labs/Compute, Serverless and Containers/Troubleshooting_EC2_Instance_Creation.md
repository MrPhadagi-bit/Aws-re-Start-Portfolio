...
When you create the instance, you will reference a user data script to configure the instance to have an **Apache web server**, a **MariaDB relational database** (a fork of MySQL), and **PHP** running on the instance. Together, these software packages installed on a single machine are often referred to as a **LAMP stack** (Linux, Apache web server, MySQL, and PHP). Using a LAMP stack is a common way to create a website with a database backend on a single machine.
...
- A provisioned lab environment with a **CLI Host** EC2 instance.
- Lab credentials (Access Key ID, Secret Access Key, and Lab Region) provided in the lab instructions.
...
3. Analyze the contents of the script:
   ...
   | **Lines 31–55** | The script invokes AWS CLI commands to look up the subnet ID, key pair name, and AMI ID values that are needed to create the EC2 instance. Notice that line 32 ends with a backslash (`\`) character. This character can be used to wrap a single command on another line in the script file. This technique is used many times in the script to make it easier to read. |
   ...
   | **Lines 154–168** | The script creates a new EC2 instance. Notice how this part of the script uses the values that were set in lines 8 and 10, and the values that were collected in lines 16–57. Notice the reference to the user data file. The entire call to create the instance is captured in a variable that is named `instanceDetails`. The contents of this variable are then echoed out to the terminal on line 177, and they are formatted for easier viewing by using a Python JSON tool. |
...
5. To display the contents of the user data script, run the following command:
...
The terminal output displays the following message:
...
1. After you identify the issue, update the script to fix the issue, and run the script again.
2. If you are prompted to delete any existing security groups or instances that were created when the script ran previously, always reply with **Y**.
3. Is the error resolved?
...
2. In the terminal window for the CLI Host instance, run the following command to install `nmap`, which is a port scanning tool:
...
After you identify and resolve the issue, in a browser, navigate to the following address. Replace `<public-ip>` with the Public IPv4 address of the new instance that you created:

```
http://<public-ip>
```

If you resolved issue #2 successfully, you should see the following message:

> **Hello From Your Web Server!**

##### Verify the User Data Script

Check the log file that shows whether the user data script ran as expected.

1. In the terminal window for the LAMP instance, run the following command to see the log file entries as they are written:
...
2. Observe the log file entries. Notice the message that is related to the installation of MariaDB and PHP. There should be no error messages.

3. You should also see messages related to files for the Café Web Application that were downloaded and extracted to this instance, such as:

   > **Create Database script completed**

4. When you are finished with your observations, to exit the `tail` utility, enter **Ctrl-C**.

   > **Tip:** To view the entire log file, run:
   >
   > ```bash
...
1. In a browser, navigate to the following address. Replace `<public-ip>` with the Public IPv4 address of the new instance that you created:
...
2. Test whether you can order items through the website:
   ...
   - Choose a few desserts to order, and then choose **Submit Order**.
   - The **Order Confirmation** page displays with line-item details.

3. Place another order for different items. Then, choose the **Order History** page. The details of both orders were captured.
...
| Command | Purpose |
|---------|---------|
| `aws configure` | Configure AWS CLI credentials and default settings |
| `view create-lamp-instance-v2.sh` | Open script in read-only mode (VI) |
| `./create-lamp-instance-v2.sh` | Run the LAMP instance creation script |
| `cat create-lamp-instance-userdata-v2.txt` | View the user data script |
| `sudo yum install -y nmap` | Install the nmap port scanner |
| `nmap -Pn <public-ip>` | Scan open ports on the target instance |
| `sudo tail -f /var/log/cloud-init-output.log` | Monitor cloud-init output in real time |
| `sudo cat /var/log/cloud-init-output.log` | View full cloud-init output |
...
- [ ] The AMI ID used in the script exists in the target region.
- [ ] The `run-instances` command references the correct region.
- [ ] Security group allows inbound traffic on port **80** (HTTP).
- [ ] Security group allows inbound traffic on port **22** (SSH).
- [ ] The `httpd` service is running on the LAMP instance.
- [ ] The user data script executed successfully (check `cloud-init-output.log`).
- [ ] The Café Web Application files were downloaded and extracted correctly.
